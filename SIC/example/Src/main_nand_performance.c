...
//	nandInit0(&MassNDisk);	
		...
		for (ii=0; ii< pSM0->uBlockPerFlash; ii++)
		{
			if (fmiCheckInvalidBlock(pSM0, ii))
...
//#define NAND_PERFORMANCE 
#ifdef NAND_PERFORMANCE 

#define ACCESS_SIZE		10 	// MB

	sysEnableCache(CACHE_WRITE_BACK);	
//	u32SecCnt = 1;	// 512B page NAND
	u32SecCnt = 4;	// 2KB page NAND	
	while(1)
	{
		nand_block_erase0(5);	
